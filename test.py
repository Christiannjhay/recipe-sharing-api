...
class CreateRecipeTestCase(unittest.TestCase):
    ...
    def tearDownClass(cls):
        try:
            ...
            cls.test_cursor.execute("DELETE FROM Recipes")
            cls.test_conn.commit()
    ...
    def test_create_recipe(self):
        # Register a user with known credentials for recipe creation
        password_to_register = '789123'
        hashed_password = generate_password_hash(password_to_register)
    ...
    def test_create_comment(self):
        ...
        if registration_response.status_code == 201:
            ...
            if token:
                ...
                headers = {'Authorization': f'Bearer {token}'}
                add_comment_response = self.app.post('/recipes/1/comments', data=json.dumps(new_comment_data),
                                                     headers=headers, content_type='application/json')
                if add_comment_response.status_code == 200:
                    add_comment_response_data = json.loads(add_comment_response.data)
                    self.assertEqual(add_comment_response_data['message'], 'Comment added successfully!')


class AddRaringTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # This method is called once before any tests are run in the class
        app.config['TESTING'] = True
...
class UpdateRecipeTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # This method is called once before any tests are run in the class
        app.config['TESTING'] = True
...
class SearchRecipeTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # This method is called once before any tests are run in the class
        app.config['TESTING'] = True
...
class SuggestionRecipeTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # This method is called once before any tests are run in the class
        app.config['TESTING'] = True
